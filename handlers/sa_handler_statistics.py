...
def save_mean_std(feat: T) -> T:
    feat_mean, feat_std = calc_mean_std(feat)
    mean_splits = torch.split(feat_mean, feat.shape[0]//2, dim=0)
    std_splits = torch.split(feat_std, feat.shape[0]//2, dim=0)
    feat_mean = torch.cat([mean_splits[0][1:], mean_splits[1][1:]], dim=0).flatten()  # remove style reference
    feat_std = torch.cat([std_splits[0][1:], std_splits[1][1:]], dim=0).flatten() # remove style reference
    feat_mean_std = torch.cat([feat_mean.unsqueeze(0), feat_std.unsqueeze(0)], dim=0)
    return feat_mean_std
...
class SharedAttentionProcessor(DefaultAttentionProcessor):
    ...
    def shared_call(
        ...
        if self.adain_queries:
            self.stats_q = save_mean_std(query)
            query = adain(query)
        if self.adain_keys:
            self.stats_k = save_mean_std(key)
            key = adain(key)
        if self.adain_values:
            self.stats_v = save_mean_std(value)
            value = adain(value)
...
class Handler:
    ...
    def register(self, style_aligned_args: StyleAlignedArgs, ):
        ...
        self.atts = init_attention_processors(self.pipeline, style_aligned_args)
